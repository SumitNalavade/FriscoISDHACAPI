...
from _lib.fakeData import fakeStudentSchedule
...
class handler(BaseHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    if(username.lower() == "john" and password.lower() == "doe"):
        ...
        self.end_headers()
        self.wfile.write(json.dumps({"schedule": fakeStudentSchedule}).encode(encoding="utf_8"))

        return

    session = getRequestSession(username, password)

    schedulePageContent = session.get("https://hac.friscoisd.org/HomeAccess/Content/Student/Classes.aspx").text
